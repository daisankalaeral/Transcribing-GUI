...
def get_duration(file):
    p = subprocess.Popen(fr'{ffprobe_bin}ffprobe -i "{file}" -show_entries format=duration',stdout=subprocess.PIPE,stderr = subprocess.PIPE,universal_newlines=True, shell = True)
    duration = float(p.stdout.readlines()[1].replace('duration=', ''))
    return duration

def get_details(file):
    p = subprocess.Popen(fr'{ffprobe_bin}ffprobe -i "{file}" -show_entries stream=duration,sample_rate,sample_fmt,channels,duration',stdout=subprocess.PIPE,stderr = subprocess.PIPE,universal_newlines=True, shell = True)
...
next_image = tk.PhotoImage(file = fr"img/next.png")
...
load_text_image = tk.PhotoImage(file = fr"img/text_icon.png")
...
slider = ttk.Scale(slider_bar, from_ = 0, to = 100, orient = HORIZONTAL, value = 0, command = slide)
